...
SWIFT: ALBPPLPW\\
% ABA: 026008044\\
% mBank BREXPLPWMBK FFC Michał Psota\\
acct PL <% @var seller-account %> &  & \tabularnewline
\cline{1-1} \cline{3-3}
\end{tabular}
\par\end{flushleft}
...
<% (dolist (item-calculations (getf env :calculated-items))
	   (format t
		   "~{\\multicolumn\{1\}\{|r|\}\{~a\} & \\multicolumn\{1\}\{>\{\\raggedleft\}p\{0.2\\paperwidth\}|\}\{~a\} & \\multicolumn\{1\}\{r|\}\{~a \} & {~d} & ~a & ~a\\ & ~a & ~a \\tabularnewline\\hline~}~%%~%"
		   item-calculations)) %>
