...
%\cline{5-8} 
&  &  & Łącznie: & <% @var zw-net-total %> \currencyShort{} & np & 0,00 PLN & <% @var zw-net-total %> \currencyShort{}\tabularnewline
\cline{5-8} 
\end{tabular}
\par\end{flushright}
...
\begin{flushright}
\textbf{Do zapłaty: <% @var gross-total %> \currencyShort{}.}\\
Słownie: <% @var words-gross-total %>, <% @var gross-total-cent %>/100.\\
<% @var payment-form %> \\%%% Forma płatności.
Uwagi: Obciążenie odwrotne. / Reverse charge.\\
\par\end{flushright}
\end{document}
